...
for fileName in os.listdir(filePath):
    tweetsDF = tweetsDF.append(
        pd.read_csv(filePath+fileName, index_col=0)
    )
...
for tickers in [
    'AAPL', 'AMZN', 'FB', 'GOOGL', 'BABA', 'WMT', '^GSPC'
]:
    print(tickers)
    ...
    newsColumns = [
        k for k in vectorizedTweets.columns.tolist() 
        if k!='label_date'
    ]
    mergedDF = merge_news_price_df(newsDF, priceDF)
    if vectorizeMethod == 'bow':
        weekendTreatedMergedDF = TreatWeekendTweets(
            mergedDF, newsColumns
        )
    elif vectorizeMethod == 'tfidf':
        # no weekend treatment for tfidf method!
